...
bool InitWindow(SDL_Window *window) {
  ...
  for (int i = 0; i < sizeof(renderers) / sizeof(renderers[0]); i++) {
    ...
    if (bgfx_init(&init)) {
      bgfx_set_view_rect(0, 0, 0, (uint16_t)kWindowWidth,
...
void PrintUuid() {
  GUID my_uuid;
  generate_uuid_v4(&my_uuid);
  RPC_CSTR uuid_str = NULL;
  if (UuidToStringA(&my_uuid, &uuid_str) == RPC_S_OK) {
    printf("uuid: %s\n", uuid_str);
    RpcStringFreeA(&uuid_str);
  } else {
    fprintf(stderr, "Error converting UUID to string.\n");
  }
}
