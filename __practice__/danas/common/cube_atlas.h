#ifndef DANAS_CUBE_ATLAS_H
#define DANAS_CUBE_ATLAS_H
...
struct AtlasRegion {
  ...
  uint32_t mask;  // encode the region type, the face index and the component
                  // index in case of a gray region
...
class Atlas {
 public:
  ...
  /// @param textureSize an atlas creates a texture cube of 6 faces with size
  /// equal to (textureSize*textureSize * sizeof(RGBA) )
  ...
  /// initialize a static atlas with serialized data	(region can be updated
  /// but not added)
  /// @param textureSize an atlas creates a texture cube of 6 faces with size
  /// equal to (textureSize*textureSize * sizeof(RGBA) )
  /// @param textureBuffer buffer of size
  /// 6*textureSize*textureSize*sizeof(uint32_t) (will be copied)
  /// @param regionCount number of region in the Atlas
  /// @param regionBuffer buffer containing the region (will be copied)
  /// @param maxRegionCount maximum number of region allowed in the atlas
  Atlas(uint16_t _textureSize, const uint8_t* _textureBuffer,
        uint16_t _regionCount, const uint8_t* _regionBuffer,
        uint16_t _maxRegionsCount = 4096);
  ~Atlas();

  /// add a region to the atlas, and copy the content of mem to the underlying
  /// texture
  uint16_t addRegion(uint16_t _width, uint16_t _height,
  ...
  /// using the supplied vertex format. v0 -- v3 |     |     encoded in that
  /// order:  v0,v1,v2,v3 v1 -- v2
  /// @remark the UV are four signed short normalized components.
  /// @remark the x,y,z components encode cube uv coordinates. The w component
  /// encode the color channel if any.
  /// @param handle handle to the region we are interested in
  /// @param vertexBuffer address of the first vertex we want to update. Must be
  /// valid up to vertexBuffer + offset + 3*stride + 4*sizeof(int16_t), which
  /// means the buffer must contains at least 4 vertex includind the first.
  /// @param offset byte offset to the first uv coordinate of the vertex in the
  /// buffer
  /// @param stride stride between tho UV coordinates, usually size of a Vertex.
  void packUV(uint16_t _regionHandle, uint8_t* _vertexBuffer, uint32_t _offset,
              uint32_t _stride) const;
  void packUV(const AtlasRegion& _region, uint8_t* _vertexBuffer,
              uint32_t _offset, uint32_t _stride) const;

  /// Same as packUV but pack a whole face of the atlas cube, mostly used for
  /// debugging and visualizing atlas
  void packFaceLayerUV(uint32_t _idx, uint8_t* _vertexBuffer, uint32_t _offset,
                       uint32_t _stride) const;
