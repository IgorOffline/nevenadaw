#ifndef DANAS_FONT_MANAGER_H
#define DANAS_FONT_MANAGER_H
...
struct FontInfo {
  ...
  float lineGap;
  /// This field gives the maximum horizontal cursor advance for all glyphs in
  /// the font.
...
class FontManager {
 public:
  ...
  FontHandle createFontByPixelSize(TrueTypeHandle _handle,
                                   uint32_t _typefaceIndex, uint32_t _pixelSize,
  ...
  FontHandle createScaledFontToPixelSize(FontHandle _baseFontHandle,
                                         uint32_t _pixelSize);
  ...
  bool addGlyphBitmap(FontHandle _handle, CodePoint _character, uint16_t _width,
                      uint16_t height, uint16_t _pitch, float extraScale,
                      const uint8_t* _bitmapBuffer, float glyphOffsetX,
                      float glyphOffsetY);
  ...
  float getKerning(FontHandle _handle, CodePoint _prevCodePoint,
